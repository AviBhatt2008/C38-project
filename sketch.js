...
function setup() {
  ...
  invisibleGround = createSprite(width*2,height - 10,width*4,10);
...
function draw()
{
  ...
  // ground as image
  imageMode(CENTER);
  ground = image(groundImage, width*2, height-10, width*4, 20);
  ...
  if(gameState === PLAY)
  {
    ...
    trex.velocityX = 5 + (frameCount / 150);
    ...
    if(obstaclesGroup.isTouching(trex) || trex.x > width*3)
    {
        gameState = END;
        dieSound.play()
    }
...
function reset()
{
  ...
  cloudsGroup.setLifetimeEach(width / 3);
